...
namespace Core
{
    ...
    void step_3([[maybe_unused]] thrust::device_vector<int> to_fix)
    {
        ...
        //     }
        // };
        // auto first_none_zero = thrust::find_if(thrust::device, histogram.begin(), histogram.end(), non_zero());
        ...
        // struct apply
        // {
        //     __device__ bool operator()(const int pixel)
        //     {
        //         return pixel+1;//roundf(((histogram[pixel] - cdf_min) / static_cast<float>(image_size - cdf_min)) * 255.0f);
        //     }
        // };
        // thrust::transform(to_fix.begin(), to_fix.end(), to_fix.begin(), apply());
    }
} // namespace Core
